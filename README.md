...
- **모듈 경계 누수** — `notification-api` 부트스트랩의 ComponentScan 이 use case 모듈 전체를 잡아 dispatch 서비스 의존성까지 빈으로 등록됨 (실제 호출은 worker 에서만). 모듈별 @Configuration 분리로 좁히는 작업이 다음 개선 항목.
